...
Prior to introducing them to help explain the subtle differences between the
test algorithms it is useful to understand the main characteristics.
For this purpose Table Reference
...
\begin{table}[h!]
    \begin{center}
      ...
      \begin{tabular}{c|c|c|c}
        \textbf{Test Algorithm } & \textbf{Pre Survey} & \textbf{Stop Criterion} &  \textbf{Stop Criterion}\\
        \hline
      ...
      \end{tabular}
      \caption{A high level comparison between the test algorithms. The merged cells mean Stop and Decision criteria are simultaneous. Otherwise multi-step.
      }
    \end{center}
  \end{table}

Pre Survey indicates a decision  $N_\mathrm{max}$, $\mathrm{ROPE}$



part of caption:\\
When the Stop Criterion and Decision Criterion cells are merged this means that both apply simultaneously.
