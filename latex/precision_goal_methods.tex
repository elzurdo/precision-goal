...
\begin{table}[h!]
    \begin{center}
      \begin{tabular}{l|c|l|l}
        ...
        Precision is the Goal & Goal & Precision $\le$ Goal & Stop then Decision \\
        Enhanced PitG & Goal & Precision $\le$ Goal \textbf{AND} & Stop $\&$ Decision \\
...
\subsubsection{Expected Stop Iteration}\label{sec:expected_stop_iteration}

Because the Bayesian HDI width shrinks as $N^{-1/2}$, the stopping iteration of precision-based algorithms can be anticipated analytically. A normal (CLT) approximation to the posterior gives a closed-form expression: for Bernoulli data with true rate $\theta$ and precision goal $\omega_{\rm goal}$,
...
\end{equation}
where $z_{*}$ is the critical value corresponding to the chosen credibility level (e.g., $z_{*}\approx 1.96$ for 95\%). The continuous analogue replaces $\theta(1-\theta)$ with the estimand variance $\sigma^2$. Because $\theta(1-\theta)$ is maximised at $\theta=0.5$, Equation~\ref{eq:pitg_stop_iteration} confirms that the fair-coin case demands the largest sample, and the expected stop iteration decreases monotonically as $\theta$ moves toward $0$ or $1$ (see Appendix~\ref{app:expected_stop} for the full derivation).
