...
We then present the stop/decision algorithms qualitatively, in pseudo code as well as analytical expressions.
...
\subsection{The Intuition Behind the Algorithms}
...
\begin{itemize}
    ...
    \item The expected {\it aleatoric uncertainty} at the stopping iteration
\end{itemize}
...
players and regulators that the dice are not loaded.

In the algorithms explored here we are interested in the quantifying the overlap
...
detail, but have the same decision criterion which we discuss next.
...
\begin{itemize}
    \item If the HDI is completely outside of the ROPE → Reject the Null Hypothesis
...
Since risk assessment is highly contextual, we defer a discussion and rather focus on the 
the necessity to collect more data as a trade-off to 
resulting in too many inconclusive outcomes. This is the main contribution of this study.
...
above in pseudo code using the HDI and ROPE boundaries as heuristics to determine the relationship between a point metric success rate $\hat{\theta}$ and the null hypothesis value $\theta_{\rm null}$.
...
In Figure \ref{fig:posteriors} we display three posteriors of different stopping points of the same experiment of Bernoulli
...
The ROPE is of width 10\% around $\theta_{\rm null}$ (i.e, 5\% on each side).
...
cherry picked example) is that the decision outcome may differ by iteration.
