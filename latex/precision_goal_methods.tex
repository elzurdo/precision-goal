...
All three algorithms rely on Bayesian heuristics that describe two key aspects of the
posterior distribution: its \textit{location} (central tendency relative to the null)
and its \textit{width} (quantifying the epistemic uncertainty of the estimate).

We begin with a discussion on how to quantify effect size of a sample parameter in terms of the location of the posterior
relative to the null hypothesis, and how to quantify precision of a parameter measurement in terms of the width
of the posterior. We laster discuss using these heuristics to define a decision rule that is shared by all three algorithms
followed by using them for stop criteria that differ across algorithms.

\subsubsection{Effect Size by Region of Practical Equivalence (ROPE)}
