...
(BFDA; \citealt{schonbrodt2018}), which plans sample size to achieve a target Bayes Factor
...
DPitG is a generic sequential stopping rule that applies to any posterior admitting a
credible interval that jointly encodes location and width, including non-conjugate and
hierarchical settings that require MCMC.
...
A ROPE that misrepresents the practically meaningful effect size will produce
systematically misleading verdicts that the precision criterion alone cannot detect;
sensitivity analyses across a range of ROPE widths are therefore recommended before
committing to a single value when no domain standard exists \citep{lakens2018}.
Appendix~\ref{app:practical_guidance} discusses principled approaches for setting it
--- including the MCID, bioequivalence standards,
...
The joint operating characteristics of DPitG can be assessed prospectively, and a
researcher can calibrate them before data collection by checking, analytically or via
simulation, that the planned $N_{\rm max}$ achieves the target conclusiveness rate at
the effect sizes of interest.
...
We now turn to the boundaries of the present work.
