The central finding of this paper is that DPitG resolves the tension inherent in
PitG: requiring both precision and decisiveness simultaneously yields a 97.7\%
...
Moreover, DPitG's conclusiveness is robust to the choice of $\omega_{\rm goal}$:
...
DPitG is a generic sequential stopping rule that applies to any posterior from which
...
The joint operating characteristics of DPitG can be assessed prospectively.
...
Notably, DPitG's conclusiveness is more sensitive to $N_{\rm max}$ than to
$\omega_{\rm goal}$ (Section~\ref{sec:goal_impact}); for the fair coin case
($\Delta_{\rm ROPE}=0.1$), the median sampling premium relative to $N_{\rm goal}$
...
Fourth, while DPitG applies in principle to any posterior from which an HDI can be
