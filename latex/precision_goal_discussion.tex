...
Unlike frequentist approaches, where conducting multiple simultaneous tests inflates the
Type~I error rate and requires corrections (e.g.\ Bonferroni) that reduce per-test
power, the Bayesian posterior is not subject to this inflation: $\omega_{\rm goal}$
and the ePitG stopping rule apply unchanged across independent simultaneous tests
without correction \citep{gelman2012, kruschke2015doing};
Appendix~\ref{app:multiple_comparisons} develops this point in full and discusses the
three types of Bayesian power analysis (prospective, retrospective, and replication)
that arise in this framework.

Of these, \textit{prospective} power analysis is most directly relevant to sample-size
planning: given a hypothesised data-generating distribution, $N_{\rm goal}$
(Equation~\ref{eq:pitg_stop_iteration}) provides a closed-form lower bound on the
samples required to achieve $\omega_{\rm goal}$; ePitG may need additional samples when
$\theta_{\rm true}$ lies near a ROPE boundary (see Section~\ref{sec:trends}).
