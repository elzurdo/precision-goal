...
We then conclude by examining trends across the full range of $\theta_{\rm true}$ values from 0.5 to 0.65, which encompasses the ROPE and its boundaries.
...
We expand this analysis to $M=2,000$ experiments, displaying outcomes in Figure \ref{fig:fair_iter_vs_rate} and summarizing key statistics in Table \ref{tab:fair_iter_vs_rate}. \footnote{This visual is inspired by \cite{kruschke2015doing}, Figure 13.6.}

\begin{figure}[h!]
  \centering
  \includegraphics[width=1\textwidth]{fair_experiments_iter_vs_rate.png}
  \caption{Outcomes of 2,000 experiments.
  Bottom panel: histograms of stop iterations $N$ color coded for each algorithm.
  Left panel: histograms of observed success rate $\hat{\theta}(N)$ at the iteration when the stop criterion is met color coded for each algorithm.
  ...
  Dashed lines: ROPE boundaries.
  \textcolor{red}{\textbf{[TK: analytical results]}}
...
Figure \ref{fig:fair_iter_vs_rate} visualises results of
2,000 experiments show that, on average,
all methods yield unbiased outcomes
(see the $\overline{\theta}$ column in Table \ref{tab:fair_iter_vs_rate}).
...
Precision is the Goal (blue circles) stops on median iteration of $\sim 599$
(and very narrow IQR 598-599),
but 61\% of decisions remain inconclusive. For ePitG (green Xs),
achieving conclusiveness requires additional sampling,
resulting in only 1.6\% inconclusive outcomes by the final iteration
($N_{\rm max}=1,500$). This manifests as a median 627 (IQR of 598-794) samples per
which is only a 4.7\% median increase compared to PitG.\footnote{The ePitG $N$ 75th percentile is 794, which is a 32.5\% increase compared to the 599 of PitG. This highlights that while the median increase is small, there is a long tail of experiments that require significantly more samples to achieve conclusiveness with ePitG.}
...
TODO: mention that the ePitG results is a hybrid of the other two.
