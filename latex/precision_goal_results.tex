...
$\theta_{\rm true}=0.5$ onwards at intervals of $0.1$. \footnote{Due to symmetry of the problem the results between $\theta_{\rm true}=0.35$ through $0.5$ would mirror the ones shown here.}
...
The ROPE top boundary is highlighted with a vertical dashed line to indicate an important inflection point.
In the case of the acceptence and rejection rates - left of the ROP top boundary acceptence is correct
and rejection incorrect, and right of it vice versa.
...
at 36.7\%. From there they all decrease to $\theta_{\rm true}$ at the upper bound of the ROPE
which serves as a minimal point in which they rise again. Whereas HDI+ROPE and ePitG
...
The acceptence and rejection rates follow similar patterns, since we consider these
only for the conclusive experiments. As seen in Section \ref{sec:fair_coin}
the HDI+ROPE start with a false positive rate of $\sim 6\%$ at $\theta_{\rm true}=0.5$
(manifested in the middle panel as the red dashed line) an reaches a max of 30\%
at the inflection point. Examining its acceptence rate (red solid line) we
see that at the inflection point it is at $\sim$10\% but quickly drops to the correct 0\%
by $\theta_{\rm true}=0.57$.

Both of the precision based methods start at no false positives at $\theta_{\rm true}=0.5$,
and there is a small rise to the inflection point which is similarly dropped quickly.

TODO: mention that the inflection point is tricky ...

In the right panel of Figure \ref{fig:conclusiveness_rates} we examine one manner of
dealing with inconclusive experiments. For each inconclusive experiment we assign a
decision of accept or reject $\theta_{\rm null}$ by checking whether the mean of the posterior ($N_{\rm success}/N_{\rm total}$)
...
We then calculate the overall acceptence and rejection rates by $\theta_{\rm true}$ values and
then flag them as \textit{correct} or \textit{incorrect} by comparing them to the ROPE boundaries.
...
The results of all 2,000 experiments shows TBD.
