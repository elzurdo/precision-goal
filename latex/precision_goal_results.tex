...
$\theta_{\rm true}=0.5$ onwards at intervals of $0.1$.\footnote{Due to symmetry of the problem the results between $0.5$ downwards would mirror the ones shown here.}
...
at 36.7\%. In the left panel of Figure \ref{fig:conclusiveness_rates} they all decrease as $\theta_{\rm true}$ nears the ROPE boundary
...
$\theta_{\rm true}=0.65$ at which ePitG and PitG are effectively the same algorithm
...
is broken down to its two components: the acceptence  and rejection rates. As
such we similar trends. In this case of $\theta_{\rm null}=0.5$ accepting the null hypothesis at
...
Both of the precision based methods start at no false positives/rejections at $\theta_{\rm true}=0.5$,
...
TODO: mention that the inflection point is tricky ...
...
is at the ROPE boudary. Within the ROPE we see that the fastest climber
is ePitG whereas PitG is somewhat lower even when the null hypothesis is true
($\theta_{\rm true}=\theta_{\rm null}=0.5$). Outside the ROPE we see that HDI+ROPE
grows as fast as ePitG at yield 100\% accuracy by $\theta_{\rm true}=0.58$, whereas
this happens for PitG only at $\theta_{\rm true}=0.61$.

The fact that HDI+ROPE yields 100\% at $\theta_{\rm true}=0.58$ accuracy, not not mean
that it does not yield bias results, which we examine in Figure \ref{fig:success_by_truth_conclusive}
which visualises the parameter value at the stop for each true value $\hat{\theta}(\theta_{\rm true})$.
In the left panel we visuliae the IQR of each algorithm and in the right panel we visualise
...
\begin{figure}[h!]
  ...
  Right: Deviations from the truth: Median minus $\theta_{\rm true}$.f
  }
