We setup the problem being addressed by examining in detail the specific case
when the null hypothesis is  and the ROPE is $[0.5, 0.55]$.
We then present general trends across the full range of $\theta_{\rm true}$ values
which encompasses the ROPE and its boundaries.
Detailed analyses of $\theta_{\rm true}=0.6$ and $\theta_{\rm true}=0.57$
scenarios are provided in Appendix A.


We begin with a detailed examination of the fair coin case to establish the problem and solution ($\theta_{\rm null}=0.5$).
We then present general trends across the full range of $\theta_{\rm true}$ values 
which encompasses the ROPE and its boundaries. Detailed analyses of $\theta_{\rm true}=0.6$ and $\theta_{\rm true}=0.57$
scenarios are provided in Appendix A (TODO Reference).
...
In contrast, the \textit{Precision is the Goal} (PitG) stop criterion is met at iteration 598, when the HDI width becomes narrower than the precision goal of 0.08. However, because the HDI still straddles the ROPE boundary, the decision is inconclusive.
...
Figure \ref{fig:fair_iter_vs_rate} visualises results of 2,000 experiments.\footnote{This visual is inspired by \cite{kruschke2015doing}, Figure 13.6.} 
...
In contrast, PitG (blue circles) consistently stops around iteration 599 with success rates tightly clustered around $\theta_{\rm true}=0.5$,
but, as we will see below in Figure \ref{fig:fair_decisions}, with a high rate of inconclusive outcomes.
ePitG (green Xs) shows a similar distribution to PitG but extends to later iterations, achieving more conclusive results while maintaining unbiased estimates.

Figure \ref{fig:fair_decisions} displays the cumulative distribution of decisions for the three algorithms.\footnote{Display inspired by top panels of Figure 13.6 in \cite{kruschke2015doing}.}
...
The rejection rate of HDI+ROPE (red dashed line in left panel) quickly assymptotes at 6\%,
while the acceptance rate (green solid line) increases, reflecting a shift from incorrect rejections to correct acceptances as more data is collected.
The gray dot-dashed line indicates the proportion of inconclusive decisions, which decreases over time as more experiments yield conclusive outcomes.
Both the precision based methods demonstrate a zero rejection rate (red dashed lines in middle and right panels), as they are designed to avoid false positives by requiring precision criteria to be met before making a decision.
For PitG (middle panel), the acceptance rate (green solid line) is appears as a Heaviside step function at iteration 598, where it jumps to 36.7\%,
while the inconclusive rate (gray dot-dashed line) drops to 63.3\%. ePitG (right panel) shows a steady increase in acceptance rate, reaching 98.4\% by iteration 1,500, with a corresponding decrease in inconclusive decisions.
The decision rates of ePitG appear to be a hybrid of the other two algorithms,
as it maintains the unbiased nature of PitG while achieving a high conclusiveness
rate similar to HDI+ROPE, albeit with a more gradual transition from inconclusive to
conclusive outcomes.
