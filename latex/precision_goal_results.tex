...
\begin{enumerate}
  ...
  \item \textbf{Slightly Loaded Coins ($\theta_{\rm true}=0.53, 0.57$)}: Weak effects exist near the ROPE boundary.
\end{enumerate}
...
\subsection{Loaded Coin ($\theta_{\rm true}=0.6$)}

This experiement represents a case where the null hypothesis is false and there is a fairly strong effect. Results for these experiments are in Figures \ref{fig:loaded0pt6_iter_vs_rate} and
...
success rate at stop of 64.4\%. This can be explained by the many experiments
which stopped fairly early resulting in a mean stop iteration of $\sim \overline{n}=268$ (with a large standard deviation of $\sigma_n=291$)
which is half the precision goal value of $N_{\theta_{\rm true}}=575.2$.

Both of the the precision based algorithms result in
unbiased outcomes. That said, PitG is still quite undecisive at a rate of 29.2\%
where ePitG is even more decicive at a rate of 99.8\% (for the fair coint it was 98.4\% decisive).
In this case the average ePitG requires only 10.7\% more samples than PitG, on average.

The apparent symmetry in the fair coin Figure \ref{fig:fair_iter_vs_rate} around
its true value of $\theta_{\rm true}=0.5$ is, as expected, not present in the loaded coin case in Figure \ref{fig:loaded0pt6_iter_vs_rate}.
This, e.g, manifests in the sharp slope of the PitG blue circles which assymptote towards the 
precision goal value of $N_{\theta_{\rm true}}=575.2$ (dashed vertical line) from above ($\hat{\theta}\ge \theta_{\rm true}$) and not from the bottom.
...
\begin{figure}[h!]
  ...
  and the Enhanced PitG as the green Xs. $\theta_{\rm true}$ is the horizontal solid line and
  the ROPE is the horiztonal dashed lines. Summary stats are in Table \ref{tab:fair_iter_vs_rate}. TK: analytical results
  }
...
\begin{figure}[h!]
  ...
  \caption{Similar to Figure \ref{fig:fair_decisions} but for $\theta_{\rm true}=0.6$. 
    It contains the same information as in Figure \ref{fig:loaded0pt6_iter_vs_rate} but focusing on
