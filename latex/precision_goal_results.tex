...
\begin{itemize}
  \item Fair coin: $\theta_{\rm true}=0.5$
  \item Highly loaded coin $\theta_{\rm true}=0.6$
  \item Slightly loaded coins $\theta_{\rm true}=0.51, \ 0.55$
\end{itemize}
...
meets both criteria at iteration 804: narrower than 0.08 precision goal and fully
within the ROPE.
...
Table \ref{tab:fair_iter_vs_rate}. Note that this visual is insipred by
Figure ?? of (cite JK), but whereas they focus on the success rate value at the
stop iteration $\hat{\theta}$ we scatter these against the iteration number.
...
that on average all methods yield unbiased outcomes, as per the
...
This display is inspired by Figure TK of JK. (TK reference to JK's graph).
...
of the promise of the Precision is the Goal in terms of resulting in unbiassed outcomes. Its main 
...
\begin{itemize}
  ...
  \item ${\rm CI}$ is the precision goal of the credible interval. In our case 0.08.
  \item $z_{*}$ is the (TK). E.g, for 95\% quantile of the standard normal
  distribution $z_{*}=z_{0.05}=1.96$
\end{itemize}

For this fair coin instances we set $p=\theta_0=\theta_{\rm true}=\frac{1}{2}$ and obtain
