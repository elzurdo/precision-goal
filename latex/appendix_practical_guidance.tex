...
Three design inputs must be fixed before data collection: the precision goal
$\omega_{\rm goal}$, the ROPE, and the maximum sample size $N_{\rm max}$.
The precision goal should satisfy $\omega_{\rm goal} \leq$ ROPE width --- ensuring
that a sufficiently narrow HDI can reach a conclusive verdict --- and directly
determines the expected stop iteration $N_{\rm goal}$
(Equation~\ref{eq:pitg_stop_iteration}); $N_{\rm max}$ is set by budget.
The ROPE is the most consequential and subjective of the three inputs.
