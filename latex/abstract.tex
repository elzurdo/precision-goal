...
of Practical Equivalence (ROPE) around the null hypothesis. This enables positive \textit{acceptance} of the null hypothesis ---
something Null Hypothesis Significance Testing cannot do. However, a wide HDI can satisfy the stopping criterion on early, unrepresentative samples,
incurring a systematic false-positive cost.
Fully decoupled methods, such as ``Precision is the Goal'' (PitG), eliminate this bias by
halting only once a target HDI width $\omega_{\rm goal}$ is reached; yet, by divorcing
the stopping rule from the decision criterion, PitG frequently yields inconclusive
outcomes, most critically when the null hypothesis is true.
...
stopping.
In fair coin experiments (ROPE: $\theta = 0.5 \pm 0.05$), PitG's
conclusiveness ranges from $82.5\%$ to $0\%$ (completely inconclusive) as $\omega_{\rm goal}$
widens from $0.06$ to $0.10$;
DPitG's conclusiveness remains essentially flat at $97.2\%$--$97.9\%$.
E.g., at $\omega_{\rm goal}=0.08$, DPitG reduces PitG's $63.3\%$ inconclusive rate to $2.3\%$
...
false-positive cost. This advantage holds broadly across both null and alternative parameter settings.
Results are demonstrated on single-group binary data; the framework extends straightforwardly
to continuous outcomes and two-group comparisons.
We provide a closed-form planning formula
($N_{\rm goal} \propto V/\omega_{\rm goal}^2$, where $V$ is the observation variance),
...
and is the method of choice whenever a reliable verdict is required and the study
can afford to collect at least $N_{\rm goal}$ observations.
