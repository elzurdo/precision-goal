...
%% This is not relevant anymore since we are now interested in trends.
\section*{Detailed Binomial Experiments}\label{sec:binomial_appendix}
...
Both precision-based algorithms yield unbiased outcomes. However, \textit{Precision is the Goal} (PitG) remains somewhat indecisive, with 29.2\% of experiments ending as inconclusive. In contrast, \textit{Decisive Precision is the Goal} (DPitG) is significantly more decisive, clarifying the outcome in 99.8\% of cases (compared to 98.4\% for the fair coin). In this scenario, DPitG requires only 10.7\% more samples than PitG on average.
...
\begin{table}[h!]\label{tab:loaded0pt6_iter_vs_rate}
  ...
  \begin{tabular}{c|c|c|c|c|c|c|c|c}
    ...
    DPitG & 0.0	& 0.998	& 0.998	& 0.002	& 634.9	& 149	& 0.6034	 & 0.0174 \\
    \hline
  \end{tabular}
...
\begin{figure}[h!]
  ...
  \caption{Cumulative distribution of decisions for $\theta_{\rm true}=0.6$. Compare with Figure \ref{fig:fair_decisions}. Note the high decisiveness of DPitG (green, right panel) compared to PitG (blue, right panel) and the early stopping bias of HDI+ROPE (red, left panel).}
...
\begin{figure}[h!]
  ...
  mean values. When using the HDI+ROPE algorithm as the stop criterion results in the red squares.
...
\begin{table}[h!]\label{tab:loaded0pt57_iter_vs_rate}
  ...
  \begin{tabular}{c|c|c|c|c|c|c|c|c}
    ...
    DPitG & 0.0035 &	0.542 &	0.545 & 	0.455 &	1127.2 &	395.9	& 0.576 &	0.017  \\
    \hline
  \end{tabular}
...
% dpitg	0.0035 &	0.5415 &	0.5450 & 	0.4550 &	1127.1755 &	395.941462	& 0.575583 &	0.017217
...
\begin{figure}[h!]
  ...
  the distribution of decisions. Left panel is for the HDI+ROPE algorithm. Right panel are both
