...
PitG eliminates the early-peeking bias of coupled methods by decoupling the stopping
and decision criteria \citep{kruschke2015doing}, but accepts high inconclusive rates
as the price: from $17.5\%$ to $100\%$ across $\omega_{\rm goal}=0.06$--$0.10$ in
the fair-coin case, and rising most steeply precisely where the null is true and a
reliable verdict is most needed.

DPitG closes this gap by imposing both requirements simultaneously: data collection
continues until $\omega_{\rm HDI} \le \omega_{\rm goal}$ \textit{and} its
position yields a conclusive verdict.
This joint requirement delivers a $2.7\times$ gain in conclusiveness over PitG at a
median cost of only 4.7\% more samples ($\theta_{\rm true}=0.5$, $\omega_{\rm goal}=0.08$,
$\Delta_{\rm ROPE}=0.1$), while preserving the zero false-reject (false-positive) rate that the HDI+ROPE algorithm
forfeits.
The gain is robust: DPitG's conclusiveness holds nearly flat ($97.2\%$--$97.9\%$)
across the same $\omega_{\rm goal}$ range, over which PitG's conclusiveness
collapses to zero, and is largest near the ROPE boundaries where evidence is
hardest to read.

Unlike NHST (see Appendix~\ref{app:nhst}), which can only reject or fail to reject
the null, the HDI+ROPE framework's decision rule enables positive \textit{acceptance} of the null
...
code\footnote{\href{https://github.com/elzurdo/dpitg}{https://github.com/elzurdo/dpitg}},
with an interactive online calculator for prospective sample-size planning and
retrospective analysis available at
