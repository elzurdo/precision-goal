...
inconclusive outcomes while preserving a zero false-positive rate, at a modest
median sampling cost --- advantages that hold broadly across precision goals and
true parameter values (Sections~\ref{sec:fair_coin}--\ref{sec:trends}).
...
here.
Both lack an explicit effect-size criterion (the ROPE); NHST cannot formally accept
the null, while Bayes Factors lack a direct precision goal.
Their limitations are illustrated in Appendices~\ref{app:nhst}
and~\ref{app:app_bayes_factors}, respectively.}; the framework extends naturally to continuous
outcomes (Appendix~\ref{app:expected_stop}) and two-group comparisons
