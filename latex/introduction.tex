...
``Decisive Precision is the Goal'' (DPitG), which recouples the stopping and decision
rules in a conservative manner.
Specifically, DPitG requires that \textit{both} the precision goal (stopping rule) and a
decisive hypothesis outcome (decision rule) be met simultaneously to cease data collection.
...
DPitG reduces PitG's inconclusive rate from 63.3\% to just 2.3\%,
at a median sampling premium of only $4\%$ relative to $N_{\rm goal}$.
Moreover, DPitG's conclusiveness is robust to the choice of precision goal: across
...
the median sampling premium relative to $N_{\rm goal}$ is zero for $\omega_{\rm goal}\le 0.07$
and rises to $41\%$ at $\omega_{\rm goal}=0.10$ (fair coin, $\Delta_{\rm ROPE}=0.1$).
This gain is most pronounced when the null is true or practically so --- precisely the
regime where PitG's inconclusive rate is highest --- while DPitG also maintains decisive
