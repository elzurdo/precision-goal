...
Examining on dichotomous data this more conservative rule substantially reduces the rate
of inconclusive outcomes (e.g., in the case of the null hypothesis being true from ~60\% to $<$2\%),
with only a moderate increase in average sample size (~22\%).
We demonstrate that ePitG is especially effective not only when the null hypothesis is true,
but also ensures decisive outcomes in scenarios where the null is false,
yielding a robust decision mechanism across experimental conditions.
